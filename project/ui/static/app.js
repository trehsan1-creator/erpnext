...
async function openPrompt(url,title){loading(true,'در حال آماده‌سازی پرامپت');try{const r=await fetch(url);if(!r.ok)throw new Error('پرامپت پیدا نشد');const text=await r.text();$('#promptTitle').textContent=title;$('#promptText').value=text;$('#promptChars').textContent=fmt(text.length)+' کاراکتر';$('#copyPrompt').innerHTML='<span>□</span> کپی کامل پرامپت';$('#promptDialog').showModal()}catch(e){toast(e.message,true)}finally{loading(false)}}
