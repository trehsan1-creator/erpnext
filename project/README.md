...
در اولین اجرا هیچ شرکت، سند یا گزارش نمونه‌ای به‌صورت خودکار ساخته نمی‌شود. ویزارد اولیه این اطلاعات را دریافت می‌کند:

- نام قانونی و تجاری شرکت
- شناسه ملی و شماره ثبت اختیاری
- نوع فعالیت خدماتی، بازرگانی یا ترکیبی
- واحد پول پایه ریال یا تومان
- عنوان و بازه شمسی سال مالی
- الگوی کدینگ استاندارد

سه الگوی کدینگ ایران شامل گروه، کل و معین آماده‌اند: خدماتی با ۶۳ حساب، بازرگانی با ۶۴ حساب و جامع با ۶۶ حساب. دارایی، بدهی، حقوق مالکانه، درآمد، بهای تمام‌شده، هزینه، مالیات و حساب‌های انتظامی همراه با ماهیت بدهکار/بستانکار ساخته می‌شوند. سیستم بعد از راه‌اندازی با داشبورد صفر و بدون عدد ساختگی باز می‌شود.

از بخش تنظیمات می‌توان با واردکردن عبارت دقیق `شروع از صفر` شرکت، کدینگ، حافظه تصمیم، Parserها، مأموریت‌ها و تمام داده‌های runtime را پاک کرد. این عملیات برگشت‌پذیر نیست.

## رابط وب «آقا»

رابط RTL، بدون CDN یا وابستگی مرورگر به سرویس خارجی اجرا می‌شود:
...
هر خطای ناشناخته یا مانع جدید که handler اختصاصی ندارد به بن‌بست یا خطای مبهم تبدیل نمی‌شود. `RecoveryEngine` مرحله، عملیات، کد خطا و context مجاز را ثبت و یک پرامپت مستقل طبق `RecoveryAIResponse` می‌سازد. پاسخ AI فقط می‌تواند مقدار پیشنهادی، Rule داده‌ای امن، درخواست انسانی یا اقدام دستی پیشنهاد کند؛ تولید و اجرای کد، SQL و تغییر مستقیم دیتابیس ممنوع است.

اگر پاسخ به اطلاعات یا تأیید انسان نیاز داشته باشد، موتور بر اساس JSON دریافتی فیلدهای لازم را در کارتابل انسانی ایجاد می‌کند. تصمیم عمومی بدون handler مجاز هرگز روی سند مالی اعمال نمی‌شود. رخدادها در `logs/recovery_audit.jsonl` و وظایف در `recovery_tasks/` ذخیره می‌شوند.

مراحل متصل یا قابل اتصال به این موتور عبارت‌اند از:

- خواندن فایل و Parser
- نرمال‌سازی و نگاشت حساب
- اعتبارسنجی سند
- پیش‌نیاز گزارش
- همگام‌سازی ERPNext
- تنظیمات شرکت و گردش تأیید
